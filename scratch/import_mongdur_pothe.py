...
Chapter: গদ্য ৯ - মংডুর পথে (বিপ্রদাশ বড়ুয়া) (Chapter ID: 639)

Imports:
- 128 MCQs (linked to Topic 7)
- 11 Short Questions (linked to Topic 6)
...
from mongdur_pothe_mcqs_part1 import mcqs_part1
from mongdur_pothe_mcqs_part2 import mcqs_part2
from mongdur_pothe_short import short_questions
from mongdur_pothe_cqs import cqs

all_mcqs = mcqs_part1 + mcqs_part2

CURRICULUM_TOPICS = [
    "পাঠ ১: 'মংডুর পথে' ভ্রমণকাহিনির প্রেক্ষাপট, ভূপ্রকৃতি ও মূল বিষয়বস্তু বিশ্লেষণ",
    "পাঠ ২: মিয়ানমারের সংস্কৃতি, ঐতিহ্য, পোশাক-পরিচ্ছদ ও খাদ্যাভ্যাস বিশ্লেষণ",
    "পাঠ ৩: আরাকান রাজ্যের ইতিহাস, সাহিত্য ও ঐতিহাসিক স্থানসমূহ",
    "পাঠ ৪: বর্মি নারীদের স্বাধীন জীবনাচার, সামাজিক মর্যাদা ও বৌদ্ধ ধর্মীয় প্রভাব",
    "পাঠ ৫: লেখক পরিচিতি, মানবিক মূল্যবোধ ও ভ্রমণ অভিজ্ঞতার সামগ্রিক মূল্যায়ন",
    "পাঠ ৬: সৃজনশীল উদ্দীপক (CQ) ও অনুধাবনমূলক প্রশ্নোত্তর অনুশীলন",
    "পাঠ ৭: পাঠ্যবইভিত্তিক বহুনির্বাচনি (MCQ) ও মূল্যায়ন"
]

def run_import():
    print("=== STARTING IMPORT FOR 'মংডুর পথে (বিপ্রদাশ বড়ুয়া)' ===")
    ...
    assert len(all_mcqs) == 128, f"Expected 128 MCQs, got {len(all_mcqs)}"
    assert len(short_questions) == 11, f"Expected 11 Short questions, got {len(short_questions)}"
    ...
    with app.app_context():
        ...
        chapter_obj = Chapter.query.filter(Chapter.subject_id == subject_obj.id, Chapter.title.like('%মংডুর পথে%')).first()
        if not chapter_obj:
            print("ERROR: Chapter 'মংডুর পথে' not found in Subject 'বাংলা ১ম পত্র'!")
            return
        ...
        assert final_total == 158
        assert final_mcq == 128
        assert final_short == 11
        assert final_cq == 19
