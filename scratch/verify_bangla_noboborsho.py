...
print("=== VERIFYING CHAPTER 640 (বাংলা নববর্ষ) ===")

# 1. Total counts
counts = c.execute("""
    SELECT question_type, COUNT(*) 
    FROM questions 
    WHERE chapter_id = 640 
...
stimulus_mcqs = c.execute("""
    SELECT id, mcq_stem 
    FROM questions 
    WHERE chapter_id = 640 AND question_type = 'mcq' AND (
        mcq_stem LIKE '%উদ্দীপকটি পড়ে%' OR 
        mcq_stem LIKE '%রাজু গ্রামে থাকে%'
    )
    ORDER BY id
""").fetchall()
...
for qid, stem in stimulus_mcqs:
    ...
    print(f"  ID {qid}: {first_line[:40]}... | {second_line[:30]}... | {q_line}")

# 3. Check Board and School Tags
print("\n--- Tagged Questions in Chapter 640 ---")
tagged_q = c.execute("""
    SELECT id, question_type, mcq_stem, short_question, cq_stem 
    FROM questions 
    WHERE chapter_id = 640 AND (
        mcq_stem LIKE '%[%]%' OR 
...
cqs = c.execute("""
    ...
    FROM questions 
    WHERE chapter_id = 640 AND question_type = 'cq'
    ORDER BY id
""").fetchall()
...
shorts = c.execute("""
    ...
    FROM questions 
    WHERE chapter_id = 640 AND question_type = 'short'
    ORDER BY id
""").fetchall()
...
# 6. Check Topics
print("\n--- Topics Verification ---")
topics = c.execute("SELECT id, title, order_num FROM topics WHERE chapter_id = 640 ORDER BY order_num").fetchall()
