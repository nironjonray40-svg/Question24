...
from toilochitrer_bhoot_mcq_part1 import mcqs_part1
from toilochitrer_bhoot_mcq_part2 import mcqs_part2
from toilochitrer_bhoot_mcq_part3 import mcqs_part3
from toilochitrer_bhoot_short import short_questions
from toilochitrer_bhoot_cq_part1 import cqs_part1
from toilochitrer_bhoot_cq_part2 import cqs_part2

all_mcqs = mcqs_part1 + mcqs_part2 + mcqs_part3
all_cqs = cqs_part1 + cqs_part2

def run_import():
    print("=== STARTING IMPORT FOR 'তৈলচিত্রের ভূত (মানিক বন্দ্যোপাধ্যায়)' ===")
    ...
    assert len(all_mcqs) == 150, f"Expected 150 MCQs, got {len(all_mcqs)}"
    assert len(short_questions) == 14, f"Expected 14 Short questions, got {len(short_questions)}"
    assert len(all_cqs) == 18, f"Expected 18 CQs, got {len(all_cqs)}"
    ...
    with app.app_context():
        ...
        chapter_obj = Chapter.query.filter(Chapter.subject_id == subject_obj.id, Chapter.title.like('%তৈলচিত্রের ভূত%')).first()
        if not chapter_obj:
            print("ERROR: Chapter 'তৈলচিত্রের ভূত' not found in Subject 'বাংলা ১ম পত্র'!")
            return
        ...
        # 6. Insert 18 Creative Questions (CQ)
        cq_count = 0
        for item in all_cqs:
            q = Question(
                class_id=class_obj.id,
