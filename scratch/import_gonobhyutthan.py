...
Chapter: গদ্য ১২ - গণঅভ্যুত্থানের কথা (সংকলিত) (Chapter ID: 642)

Imports:
- 152 MCQs (linked to Topic 7)
- 9 Short Questions (linked to Topic 6)
- 19 Creative Questions (linked to Topic 6)
Total: 180 Questions
"""
...
from gonobhyutthan_mcqs_part1 import mcqs_part1
from gonobhyutthan_mcqs_part2 import mcqs_part2
from gonobhyutthan_short import short_questions
from gonobhyutthan_cqs_part1 import cqs_part1
from gonobhyutthan_cqs_part2 import cqs_part2

all_mcqs = mcqs_part1 + mcqs_part2
all_cqs = cqs_part1 + cqs_part2

CURRICULUM_TOPICS = [
    "পাঠ ১: 'গণঅভ্যুত্থানের কথা' প্রবন্ধের প্রেক্ষাপট, শাসনব্যবস্থা ও শাসকের দায়িত্ব-কর্তব্য",
    "পাঠ ২: ১৯৬৯-এর গণঅভ্যুত্থানের প্রেক্ষাপট, তাৎপর্য ও আত্মদানকারী শহিদবৃন্দ",
    "পাঠ ৩: ১৯৯০-এর গণঅভ্যুত্থান, স্বৈরশাসনের পতন ও গণতান্ত্রিক আকাঙ্ক্ষা",
    "পাঠ ৪: ২০২৪-এর জুলাই গণঅভ্যুত্থান (ছাত্র-জনতার অভ্যুত্থান), বৈষম্যবিরোধী আন্দোলন ও নতুন ইতিহাস",
    "পাঠ ৫: মানবিক, বৈষম্যহীন ও অন্তর্ভুক্তিমূলক বাংলাদেশ গড়ার প্রত্যয়",
    "পাঠ ৬: সৃজনশীল উদ্দীপক (CQ) ও অনুধাবনমূলক প্রশ্নোত্তর অনুশীলন",
    "পাঠ ৭: পাঠ্যবইভিত্তিক বহুনির্বাচনি (MCQ) ও মূল্যায়ন"
]

def run_import():
    print("=== STARTING IMPORT FOR 'গণঅভ্যুত্থানের কথা (সংকলিত)' ===")
    ...
    assert len(all_mcqs) == 152, f"Expected 152 MCQs, got {len(all_mcqs)}"
    assert len(short_questions) == 9, f"Expected 9 Short questions, got {len(short_questions)}"
    assert len(all_cqs) == 19, f"Expected 19 CQs, got {len(all_cqs)}"
    ...
    with app.app_context():
        ...
        chapter_obj = Chapter.query.filter(Chapter.subject_id == subject_obj.id, Chapter.title.like('%গণঅভ্যুত্থানের কথা%')).first()
        if not chapter_obj:
            print("ERROR: Chapter 'গণঅভ্যুত্থানের কথা' not found in Subject 'বাংলা ১ম পত্র'!")
            return
        ...
        # 6. Insert 19 Creative Questions (CQs)
        cq_count = 0
        for item in all_cqs:
            q = Question(
                class_id=class_obj.id,
        ...
        assert final_total == 180, f"Expected 180, got {final_total}"
        assert final_mcq == 152, f"Expected 152, got {final_mcq}"
        assert final_short == 9, f"Expected 9, got {final_short}"
        assert final_cq == 19, f"Expected 19, got {final_cq}"
